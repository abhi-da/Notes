...
\begin{enumerate}
    \item[(\textbf{a})] converges if $\limsup_{n \to \infty} \left| \frac{a_{n+1}}{a_n} \right| < 1$,
    \item[(\textbf{b})] diverges if $\left| \frac{a_{n+1}}{a_n} \right| \geq 1$ for all $n \geq n_0$, where $n_0$ is some fixed integer.
\end{enumerate}
\begin{frame}{Inequalities for Root and Ratio Limits}
