...
\textbf{Question in propositional logic form:}  
Show that 
\[
P \Rightarrow (P + Q)
\]
is always true (a tautology), using $+$ for OR, $\cdot$ for AND, and $'$ for NOT.

\textbf{Solution:} First, convert the implication using the formula 
\[
...
Using the associative law for OR:
\[
P' + (P + Q) = (P' + P) + Q \quad \text{(Associative Law)}
\]
\end{frame}
...
\[
(P' + P) + Q = 1 + Q \quad \text{(Annulment Law, $P' + P = 1$)}
\]

Again, by the annulment law for OR:
\[
1 + Q = 1
\]
