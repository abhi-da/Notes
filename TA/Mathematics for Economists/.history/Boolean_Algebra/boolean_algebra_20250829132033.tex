...
\begin{frame}{For statements $P$ and $Q$, show that $(P \cdot \overline{Q}) \cdot (P \cdot Q)$ is a contradiction}

Start by converting the statement into Boolean expressions:  
\[
(P \cdot \overline{Q}) \cdot (P \cdot Q)
\]

Using the commutative law:
\[
(P \cdot \overline{Q}) \cdot (Q \cdot P)
\]
...
By the complement law, $\overline{Q} \cdot Q = 0$, so we get:
\[
P \cdot 0 \cdot P = 0
\]

\alert{Hence, $(P \cdot \overline{Q}) \cdot (P \cdot Q)$ is always false, so it is a contradiction.}
\end{frame}
