...
\begin{frame}{NOT Operator}
    The NOT operator is denoted by a bar over the variable ($\overline{A}$) or an apostrophe ($A'$).
    \begin{itemize}
        \item $\overline{A} = Y$ or $A' = Y$
...
\begin{frame}{Other Important Laws}
    \begin{itemize}
        ...
        \item \textbf{Complement Law}: $A + \overline{A} = 1$, $A \cdot \overline{A} = 0$
        \item \textbf{Involution Law}: $\overline{\overline{A}} = A$
