...
\begin{frame}{For statements $P$, $Q$, and $R$, show that $((P \Rightarrow Q) \wedge (Q \Rightarrow R)) \Rightarrow (P \Rightarrow R)$ is a tautology}
First, convert implications to Boolean expressions using 
\[
A \Rightarrow B \equiv A' + B \quad \text{(Implication Law)}.
\]

Hence,
\[
((P \Rightarrow Q) \wedge (Q \Rightarrow R)) \Rightarrow (P \Rightarrow R) \equiv ((P'+Q)(Q'+R)) \Rightarrow (P'+R)
\]

Again, apply the implication law:
\[
((P'+Q)(Q'+R))' + (P'+R)
\]
...
\[
(P'+Q)' + (Q'+R)' + P' + R = (P \cdot Q') + (Q \cdot R') + P' + R
\]

Rewriting terms (commutative law):
\[
PQ' + QR' + P' + R
\]
...
Finally, using the complement law $P'+P = 1$, $R'+R=1$ and annulment law $1 + X = 1$:
\[
1
\]

\alert{Thus, $((P \Rightarrow Q) \wedge (Q \Rightarrow R)) \Rightarrow (P \Rightarrow R)$ is always true, so it is a tautology.}
\end{frame}
