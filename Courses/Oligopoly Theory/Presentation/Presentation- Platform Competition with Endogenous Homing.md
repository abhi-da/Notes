...
Now when $u_2^B>u_2^A$, all firms join Platform B, but there may be some firms who are keen to multi-home. In this case, either, $n_2^B=n_2^M=n_2^X$ or $n_2^B>n_2^M=n_2^X$. There are no distinct firms in Platform A. So if there are no distinct firms in Platform A, there's no benefit to the consumers, and they will shift to Platform B. And because there are no consumers in Platform A, and due to the restriction, $s_i<p_i^X$, the firms don't find it profitable to multi-home. 
